...
class ListAction
{
    public function run(Action $action)
    {
        ...
        $this->_show($action->directory, $files, $totals);
    }

    private function _show(string $directory, $files, $totals)
    {
        QxSmarty::assign('files', $files);
        QxSmarty::assign('totals', $totals);
        QxSmarty::assign('directory', $directory);
        QxSmarty::assign('buttons', $this->_get_buttons($directory));
        QxSmarty::display('list');
    }

    private static function _get_buttons ($dir)
    {
        $buttons = array
        (
