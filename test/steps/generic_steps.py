...
def check_success_with_binary_result(step):
    assert world.result == 0, "result was {0}\nout:{1}\nerr:{2}".format(world.result, "".join(world.output), "".join(world.stderr))
    fp = open("src/result.html", "w")
    fp.write(world.output)
    fp.close()
    assert ord(world.output[0]) == 80, "no binary result: %d" % ord(world.output[0])

@step(r'I expect success and result containing ' + Expressions.search )
def check_success_with_result(step, expected_data):
    assert world.result == 0, "result was {0}\n{1}\nout: {2}\nerr: %s".format(world.result, "".join(world.output), "".join(world.stderr))
